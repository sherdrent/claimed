...
struct Temu: View {
    ...
    var body: some View {
            ...
            ScrollView {
                VStack(spacing: 20) {
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        Text("Lawsuit")
                            .font(.headline)
                        Text("Temu is facing allegations in individual arbitration that its mobile app collects excessive user data—including biometric information, location, and camera/microphone access—without proper disclosure or consent.")
                            .font(.body)
                            .foregroundColor(.gray)
                        Spacer()
                        Text("Eligibility")
                            .font(.headline)
                        Text("You may be eligible to participate if you created a Temu account or made purchases through the Temu mobile app. Some users may receive up to $5,000, depending on eligibility and case outcome.")
                            .font(.body)
                            .foregroundColor(.gray)
                    }
                    .padding(.horizontal)
        ...
        .sheet(isPresented: $showSafari, onDismiss: {
            ...
            SafariView(url: URL(string: "https://lantern.labaton.com/case/temu")!)
        }
        
        // Alert after Safari closes - UPDATED
        .alert("Did you submit a claim for Temu Privacy Concerns?", isPresented: $showAlert) {
            Button("Yes") {
                walletManager.addClaim(
                    title: "Temu Privacy Concerns",
                    amount: 5000.0,
                    imageName: "pur16"
                )
                print("✅ User said Yes for Temu - Added to wallet")
            }
            Button("No") {
                print("❌ User said No for Temu")
            }
        }
    }
...
struct Temu_Previews: PreviewProvider {
    static var previews: some View {
        Temu(walletManager: WalletManager())
